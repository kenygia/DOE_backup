...
class CentreVille extends Batiment
{    
  ...
  { 
		...
		$this->_name='Centre Ville';
		$this->_tmpProduction=5;
		$this->_qtyProduction=1+$lvl;
		$this->_production='Or';
		$this->_prixConstruction=array('Bois' =>(int)$lvl*6,'Nouriture'=>(int)$lvl*6 );
        $this->_tmpConstruction=pow((int)$lvl,2);
		$this->_race=1;
		$this->_representation=' <img src="img/inn.svg" border="0"/> ';
    $this->_lvlMax=5;
  }
    elseif($lvl < 0)
  {
    parent::__construct($joueur,1);
    $this->_name='CentreVille';
    $this->_tmpProduction=5;
    $this->_qtyProduction=1+$lvl;
    $this->_production='Bois';
    $this->_prixConstruction=array('Bois' =>(int)$lvl*6,'Nouriture'=>(int)$lvl*6);
    $this->_tmpConstruction=pow((int)$lvl,2);
    $this->_race=1;
    $this->_representation=' <img src="img/inn.svg" border="0"/> ';
        ...
        <td>
        	<table border =4 cellspacing=4 cellpadding=4 widh=80%>
        		<tr>
        			...
        			<td> production : <img src=\"img/or.png\" width=\"40\"/> </td>
           		</tr>
        		<tr>
